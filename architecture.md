...
## 六、VM 条件评估（42.194.162.251）

| 条件 | 状态 | 说明 |
|------|------|------|
| **Ubuntu 24.04** | ✅ | 系统就绪 |
| **CPU** | ⚠️ 4 核 | 够用，但 Ollama + Embedding 可能争抢 |
| **内存** | ⚠️ 1.5G 可用（总量 3.6G） | **瓶颈**，同时跑 Qdrant + Embedding + Web 会紧张 |
| **磁盘** | ⚠️ 3G 可用（总量 40G） | **瓶颈**，安装模型和大包可能不够 |
| **Docker** | ✅ 29.2.1 | 可启动容器 |
| **Python 3.12** | ✅ | 基础环境 |
| **Ollama + deepseek-v4-pro:cloud** | ✅ **端口 11434** | 本地可调用 `/api/tags`，但 `/api/generate` 需授权（待配置） |
| **有运行的无关服务** | ⚠️ Jellyfin / webhook | 占用内存和端口 |

### 建议：双机分工

```
┌──────────────────────────────┐     ┌──────────────────────────────┐
│  当前沙箱（生产构建环境）      │     │  VM 42.194.162.251（推理环境） │
│  13G 内存 / 49G 磁盘        │     │  1.5G 内存 / 3G 磁盘         │
│                              │     │                              │
│  ● FastAPI 服务             │     │  ● Ollama deepseek-v4-pro    │
│  ● 元数据提取 + 分块        │     │  ● Embedding 模型（需安装）   │
│  ● 向量检索 + BM25         │     │  ● Qdrant（Docker 可选）      │
│  ● RRF 融合重排            │     │                              │
│  ● Streamlit 前端          │     │                              │
└──────────────────────────────┘     └──────────────────────────────┘
       │                                          │
...
| 问题 | 说明 |
|------|------|
| **Ollama auth** | `/api/generate` 返回 401，需要配置 API Key 或开放本地访问 |
| **磁盘扩容** | 3G 剩余不够安装 Embedding 模型（bge-large-zh 约 2.7G） |
| **是否双机部署** | 或是把检索服务也部署到这台 VM（需清理 Jellyfin 腾空间） |
